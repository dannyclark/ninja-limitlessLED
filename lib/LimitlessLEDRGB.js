...
var rgbwWhiteLightGroupCommands = {
...
function LimitlessLEDRGB(driver, lightGroup) {
  ...
  this.D = lightGroup.colorType == 'rgb' ? 1011 : (lightGroup.colorType == 'rgbw' ? 1013 : 1012)
  // 1011 LimitlessLED RGB device id
  // 1012 LimitlessLED White device id
  // 1013 LimitlessLED RGBW device id
...
LimitlessLEDRGB.prototype.write = function(data) {
  ...
  if ( data[0] == '{' ) {
    ...
    } else if ( this.lightGroup.colorType == 'rgbw' ) {
      if ( on ) {
        var self = this;
	// docs say highest is 0x3b i.e. 59
	// lowest that seems to work is 0x02 i.e. 2
	// so convert a number between 0 and 254 to a number between 2 and 59
	rgbwBrightness = parseInt(parsed.bri / 254 * 57) + 2
        self.sendRgbwWhiteGroupCommand( 'on', function() {
          self.sendRgbwBrightnessCommand(rgbwBrightness);
        } );
      } else {
        this.sendRgbwWhiteGroupCommand( 'off' );
      }
    } else {
      if ( on ) {
        var self = this;
...
LimitlessLEDRGB.prototype.sendRgbwWhiteGroupCommand = function(cmd, callback) {
  var cmdByte = rgbwWhiteLightGroupCommands[this.lightGroup.number][cmd];
  sendUDPCommand(cmdByte, valByteNA, this.driver._opts.ipAddress, this.driver._opts.port, callback);

};

LimitlessLEDRGB.prototype.sendRgbwBrightnessCommand = function(brightness, callback) {
  sendUDPCommand(0x4E, brightness, this.driver._opts.ipAddress, this.driver._opts.port, callback);
};
